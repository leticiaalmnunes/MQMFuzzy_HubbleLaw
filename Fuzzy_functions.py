...
def cria_fuzzy_tri_iso (lista_y, meio_diametro):
    """Cria, a partir de uma lista ou vetor de pontos e uma lista com seus respectivos diâmetros divididos por 2,
    números fuzzy triangulares isóceles.
    
    Args:
        lista_y: lista ou vetor contendo as coordenadas y dos pontos.
        meio_diametro:
    
    Returns:
        triplas: lista de tuplas que representam os números fuzzy.
    """
...
def exibe_linfunc_fuzzy (lista_solucoes):
    """"
    Descrição
    ...
    for i, sol in enumerate(lista_solucoes):
...
def plota_ajuste_fuzzy_2D(df, solucoes,
                          titulo=None, nome_x = None, nome_y = None, label_dado = None,
                          cores = ['purple', 'navy', 'violet', 'lightblue'], ticks_x=[],
                          sep_solutions = False, dimension = [10,6], quality = 200,
                          save_fig = False, display_function = False):
    """
    ...
    Args:
        ...
        solucoes: Conjunto de soluções do sistema linear fuzzy.
        
        titulo (opcional): Título que será exibido no gráfico e, caso 'save_fig==True', na figura salva. String. Valor padrão: None.
        
        nome_x (opcional): Nome dado ao eixo x. String. Valor padrão: None.
        
        nome_y (opcional): Nome dado ao eixo y. String. Valor padrão: None.
        
        label_dado (opcional): Nome dado aos dados na legenda. String. Valor padrão: None.
        
        cores (opcional): Lista de até 4 cores da biblioteca matplotlib que serão utilizadas para plotar o(s) gráficos.
        Valor padrão: ['purple', 'navy', 'violet', 'lightblue']
        
        ticks_x (opcional): Altera os valores dos dados que são exibidos no eixo x.
        ...
        sep_solutions (opcional): Argumento que indica se as soluções devem ser exibidas em gráficos separados.
        Booleano. Valor padrão: False.
        
        dimension (opcional): Dimensão do gráfico (2 dimensões). Lista. Valor padrão: [10,6].
        
        quality (opcional): Qualidade da imagem (dpi). Inteiro. Valor padrão: 200.
        
        save_fig (opcional): Argumento que indica se devem ser geradas imagens (png) para os gráficos obtidos.
        As imagens serão salvas no mesmo diretório no qual a função for utilizada. Booleano. Valor padrão: False.
        
        display_function (opcional): Argumento que indica se as equações das soluções devem ser exibidas. Booleano. Valor padrão: False.
    """
    ...
    def info(titulo_graf):
        ...
        plt.title(titulo_graf, fontsize='xx-large')
        ...
        plt.xlabel(nome_x, fontsize='x-large')
        plt.xticks(fontsize='large')
        plt.ylabel(nome_y, fontsize='x-large')
...
def plota_ajuste_fuzzy_3D(df, solucoes, titulo=None, nome_x = 'x', nome_y = 'y', label_dado = 'dados',
                          cores = ['purple', 'navy', 'violet', 'lightblue'], azimute = 245,  elevacao = 45,
                          dimensao = [10,6], qualidade = 200, save_fig = False):
    """
    Exibe o gráfico do ajuste linear fuzzy por quadrados mínimos a partir de um conjunto de dados.
    ...
    Args:
        ...
        nome_x (opcional): Nome dado ao eixo x. String. Valor padrão: x.
        
        nome_y (opcional): Nome dado ao eixo y. String. Valor padrão: y.
        
        label_dado (opcional): Nome dado aos dados na legenda. String. Valor padrão: dados.
        ...
        azimute (opcional): Ângulo da visão lateral do gráfico. Valor padrão: 245
        
        elevacao (opcional): Ângulo da visão superior do gráfico. Valor padrão: 45
        
        dimensao (opcional): Dimensão do gráfico (2 dimensões). Lista. Valor padrão: [10,6].
        
        qualidade (opcional): Qualidade da imagem (dpi). Inteiro. Valor padrão: 200.
    ...
    for sol in range(len(solucoes)):
        fig = plt.figure(figsize=dimensao, dpi=qualidade)
        
        # CRIANDO OS EIXOS
        ax = fig.add_subplot(111, projection='3d')
        ax.view_init(azim=azimute, elev=elevacao)
        ...
        ax.scatter(X,Y,Z, color='#333333', marker='o', label=label_dado, alpha=1)
        ...
        # PICO
        ax.plot(xs, y1, z1, color='#000000', label='Pico')
        
        # BASE
        ax.plot(xs, y0, z0, color=cores[sol]) # Inf
        ax.plot(xs, y2, z0, color=cores[sol],  label=f'{c+1}° ajuste obtido') # Sup
            
        ax.plot_surface(np.array([xs, xs]), np.array([y0,y1]), np.array([z0, z1]), color=cores[sol], alpha=0.3) # Inf
        ax.plot_surface(np.array([xs, xs]), np.array([y1, y2]), np.array([z1, z0]), color=cores[sol], alpha=0.3) # Sup


        # INFORMAÇÕES
        plt.title(titulo, fontsize='large')

        plt.xlabel('\n'+nome_x)
        plt.ylabel('\n\n'+nome_y)
        ax.set_zlabel('Pertencimento')
        
        plt.legend(loc='best', fontsize='large')
        plt.tight_layout()
        
        if save_fig:
            plt.savefig(f'Graph3D_{sol+1}_{titulo}', bbox_inches='tight')
